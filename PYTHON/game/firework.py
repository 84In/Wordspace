...
class BulletFlyUp():  # Viên đạn bay lên trước khi nổ
    ...
    def draw(self):
        pygame.draw.circle(DISPLAYSURF, self.color, (int(
            self.x), int(self.y)), int(self.size))  # Vẽ viên đạn
        # Vẽ từng chấm
        for dot in self.dots:
            dot.draw()
...
class FireWork():  # Quả pháo hoa
    ...
    def update(self):
        for bullet in self.bullets:  # update từng viên đạn
            bullet.update()
            self.dots.append(
                Dot(bullet.x, bullet.y, bullet.size, bullet.color))
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
        # Tạo (những) viên đạn bay lên sau khoảng thời gian xác đinh
        ...
        for bulletFlyUp in bulletFlyUps:
            if bulletFlyUp.speed <= 0:  # Viên đạn bay lên đạt độ cao tối đa
                # Tạo quả pháo ngay vị trí viên đạn
                fireWorks.append(FireWork(bulletFlyUp.x, bulletFlyUp.y))
                bulletFlyUps.pop(bulletFlyUps.index(
                    bulletFlyUp))  # Xoá viên đạn đó
